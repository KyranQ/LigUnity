...
def main():
    ...
    for cfg in csv_configs:
        ...
        for row in rows:
            ...
            if "Proasis ID" in row and not row["Proasis ID"]:
                extra_label_rows.append((target, smiles, affinity))
                continue
    ...
    label_list = list(label_entries.values())
    for target, smiles, affinity in extra_label_rows:
        label_list.append(
            {
                "pockets": [target],
                "uniprot": target,
